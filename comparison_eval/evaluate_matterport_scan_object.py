...
from tag_mapping.datasets.matterport import read_matterport_object_bounding_boxes


def evaluate_matterport_scan_object_localizations(
    params: Dict,
    scan_dir: Union[str, os.PathLike],
    om3d_region_instances_dir: Union[str, os.PathLike],
    lattice_graph_path: Union[str, os.PathLike],
    output_dir: Union[str, os.PathLike],
    logger: Optional[logging.Logger] = None,
) -> None:
    """
    Evaluates the tag map object localizations on a matterport scan by comparing
    the localized regions against the labeled mpcat40 ground-truth bounding boxes.
    ...
    Args:
        ...
        om3d_region_instances_dir: Path to the direction
            containing the OpenMask3D region instance outputs.
    ...
    # Go through and find scan name files in the openmask3d instance outputs directory
    region_instance_filenames = []
    for filename in os.listdir(om3d_region_instances_dir):
        if scan_name in filename:
            region_instance_filenames.append(filename)
    if len(region_instance_filenames) == 0:
        raise FileNotFoundError(
            f"No region instance files found for scan {scan_name} in {om3d_region_instances_dir}"
        )
    else:
        logger.info(
            f"found the following region instance files for scan {scan_name}:\n" + \
            "\n\t".join(region_instance_filenames)
        )

    # Load the labeled ground-truth bounding boxes
    label_gt_boxes = read_matterport_object_bounding_boxes(
        house_file_path, category_taxonomies=("mpcat40",)
    )["mpcat40"]
    ...
    # Aggregate region instances for the scan
    instance_preds = []
    instance_min_max_bounds = []
    label_to_pred_ind = None

    for fname in region_instance_filenames:
            
        with open(os.path.join(om3d_region_instances_dir, fname), 'rb') as f:
            data = pickle.load(f)
        
        instance_preds.append(data["instance_preds"])
        instance_min_max_bounds += data["instance_min_max_bounds"]
        
        # This assumes that label_to_ind is the same for all regions instance preds generated
        if label_to_pred_ind == None:
            label_to_pred_ind = data["label_to_ind"]
            
    instance_preds = np.concatenate(instance_preds, axis=0)
    instance_min_max_bounds = tuple(instance_min_max_bounds)

    logger.info(f"aggregated region instances")


    # print(instance_preds.shape)
    # print(len(instance_min_max_bounds))
    ...
    for label in tqdm(label_gt_boxes.keys()):
        ...
        label_instance_inds = np.where(instance_preds == label_to_pred_ind[label])[0]

        p_boxes, p_box_confidences, p_box_tags = ([], [], [])
        for ind in label_instance_inds:
            min_bound, max_bound = instance_min_max_bounds[ind]
            p_box = o3d.geometry.AxisAlignedBoundingBox(min_bound, max_bound)
            p_box = expand_box_to_min_side_length(
                p_box, params["min_proposal_box_length"])
    ...
    for label, gt_boxes in label_gt_boxes.items():
        ...
        for gt_box in gt_boxes:
            inds = assign_label_box_lattice_graph_nodes(
                ...
                # NOTE: ENABLE inflation for object box label assignment since objects could be very small
...
if __name__ == '__main__':
    ...
    OM3D_REGION_INSTANCES_DIR = "/home/rsl_admin/openmask3d/labeled_instance_outputs/object-test"
    LATTICE_GRAPH_PATH = "/media/rsl_admin/T7/matterport/lattice_graphs/2t7WUuJeko7_lattice_graph.pkl"
    OUTPUT_DIR = "/home/rsl_admin/openmask3d/eval_output/test_output"

...
def os_matterport_points(filepath):
    data_3d = torch.load(filepath)
    points, colors, _ = data_3d
    normals = np.zeros_like(points) # openscene matterport 3D data data doesn't have normals
    return points, colors, normals

def process_file(filepath):
    coords, colors, normals = os_matterport_points(filepath)
